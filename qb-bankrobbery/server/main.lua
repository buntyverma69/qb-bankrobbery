...
RegisterNetEvent('qb-bankrobbery:server:recieveItem', function(type)
    ...
    if type == "small" then
        ...
        if tierChance < 50 then tier = 1 elseif tierChance >= 50 and tierChance < 78 then tier = 2 elseif tierChance >= 78 and tierChance < 95 then tier = 3 else tier = 4 end
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                     local item = Config.LockerRewards["tier"..tier][math.random(#Config.LockerRewards["tier"..tier])]
                     local itemAmount = math.random(item.maxAmount) + 1
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                    ...
                    -- ply.Functions.AddItem(item.item, itemAmount)
                    -- TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item.item], "add")
					Player.Functions.AddMoney('cash', math.random(6000, 7000), "robbery-bank")
                elseif Config.RewardTypes[itemType].type == "money" then
				...
				Player.Functions.AddMoney('cash', math.random(7000, 8000), "robbery-bank")
            end
        else
